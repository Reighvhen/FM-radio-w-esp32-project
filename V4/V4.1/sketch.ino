...
void loop()
{
  ...
  {
    // analogRead() is never perfectly stable — even with the knob completely still,
    // it fluctuates by a few counts due to electrical noise. If we called
    // rx.setFrequency() every loop cycle, the chip would constantly be retuning
    // and never have time to actually lock onto a station and play audio.
    
    // To fix this, we only call rx.setFrequency() when the new frequency differs
    // from the last frequency we set by more than 10 units. Anything less than 10
    // is considered electrical noise and gets ignored. Only intentional knob
    // movements cause a meaningful change greater than 10, triggering a retune.

    // lastFreq remembers the last frequency we actually set, giving us something
    // to compare against each cycle to detect real vs noise changes.
    int newFreq = map(analogRead(32), 0, 4095, 8750, 10800);
